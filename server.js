...
app.post('/signupEngineer', function(req, res) {
    ...
    engineer.create({fullName: fullname, userName: username, siteLocation: location, phoneNumber: phonenumber, password: hashedPassword,}).then(function(){
...
app.post('/signupWorker', function(req, res) {
    ...
    worker.create({fullName: fullName , userName : username , password : hashedPassword , experienceLevel : experienceLevel , expectedSalary : expectedSalary , phoneNumber : phoneNumber , role : role })
         .then(function(){
...
const authenticateWorker = function(req, res, next){
	...
	jwt.verify(token, SECRET_KEY, (err, data) => {
		...
		worker.findOne({where: {userName: username}}).then( (user) => {
            //console.log(user)
			if(!user){
 ...
 app.get('/workerPage', authenticateWorker, function(req, res) {
	...
	//console.log(user)
	worker.findOne({where: {id: user.id}}).then(function(user){
		return res.send({fullName: user.fullName, userName: user.userName, experienceLevel : user.experienceLevel , expectedSalary : user.expectedSalary , phoneNumber : user.phoneNumber , role : user.role });
	}).catch(function(err){
		return res.status(500).send(err);
	})
 });
 ...
 app.get('/engineerPage', authenticateEngineer, function(req, res) {
	const user = req.body.user;
	engineer.findOne({where: {id: user.id}}).then(function(user){
